...
/// True when `path` opens with [`claude_md::SENTINEL`].
///
/// Any read failure answers `false`: refusing to overwrite a file we cannot
/// inspect is the conservative branch.
fn is_generated_claude_md(path: &Path) -> bool {
    std::fs::read_to_string(path).is_ok_and(|s| {
        s.lines()
            .next()
            .is_some_and(|first| first.trim_start().starts_with(claude_md::SENTINEL))
    })
}

/// Write `CLAUDE.md` into `out`, unless a foreign one is already there.
///
/// The rest of the tree is overwritten unconditionally, but `CLAUDE.md` is the
/// one filename an agent harness loads as standing instructions — replacing a
/// hand-written one changes behaviour, not just text. A file we did not
/// generate is left alone with a warning; it never fails the render.
fn write_claude_md(out: &Path, owner: &str, repo: &str, width: usize) -> std::io::Result<()> {
    let path = out.join("CLAUDE.md");
    // `symlink_metadata`, not `exists`: `exists` follows symlinks, so a
    // *dangling* symlink at this path would read as absent and the write
    // below would create the link's target — potentially outside `out`.
    // `symlink_metadata` reports the link itself, so a dangling symlink is
    // correctly treated as "something is here" and left to the foreign-file
    // branch below.
    if path.symlink_metadata().is_ok() && !is_generated_claude_md(&path) {
        tracing::warn!(
            path = %path.display(),
            "CLAUDE.md was not generated by meta-fetch; leaving it untouched"
        );
        return Ok(());
    }
    std::fs::write(path, claude_md::render(owner, repo, width))
}
...
pub fn render_tree(conn: &Connection, out: &Path) -> anyhow::Result<()> {
    ...
    write_readme_doc(conn, out, &meta)
}

/// Write `README.md` into `out`: entity counts and sync metadata.
fn write_readme_doc(
    conn: &Connection,
    out: &Path,
...
mod tests {
    ...
    /// Render a minimal tree into `dir`.
    fn render_minimal(dir: &Path) {
        let conn = store::open_in_memory().unwrap();
        store::repo_meta::ensure(&conn, "o", "r").unwrap();
        store::issues::upsert_issue(&conn, &issue("I1", 1, false)).unwrap();
        render_tree(&conn, dir).unwrap();
    }

    #[test]
    fn writes_claude_md() {
        let tmp = tempfile::tempdir().unwrap();
        render_minimal(tmp.path());
        let doc = std::fs::read_to_string(tmp.path().join("CLAUDE.md")).unwrap();
        assert!(doc.starts_with(claude_md::SENTINEL), "{doc}");
        assert!(
            doc.contains("o/r"),
            "owner/repo must be interpolated: {doc}"
        );
    }

    #[test]
    fn preserves_foreign_claude_md() {
        let tmp = tempfile::tempdir().unwrap();
        ...
        assert_eq!(
            ...
            "# My own notes\n",
            "a hand-written CLAUDE.md must never be replaced"
        );
    }

    #[test]
    fn overwrites_own_claude_md() {
        let tmp = tempfile::tempdir().unwrap();
        render_minimal(tmp.path());
        let path = tmp.path().join("CLAUDE.md");

        // Truncate our own output to a bare sentinel line; the next render
        // must recognise it and restore the full document.
        std::fs::write(&path, format!("{} -->\n", claude_md::SENTINEL)).unwrap();
        render_minimal(tmp.path());

        let doc = std::fs::read_to_string(&path).unwrap();
        assert!(
            doc.contains("## Issue frontmatter"),
            "our own output must be rewritten, got: {doc}"
        );
    }

    #[test]
    fn unreadable_claude_md_does_not_fail_render() {
        let tmp = tempfile::tempdir().unwrap();
        // A directory at that path cannot be read as a string.
        ...
        render_tree(&conn, tmp.path())
            .expect("an unreadable CLAUDE.md must warn and skip, never fail the render");
        assert!(tmp.path().join("issues/0001.md").exists());
    }

    #[cfg(unix)]
    #[test]
    fn dangling_claude_md_symlink_is_not_followed() {
        // `path.exists()` follows symlinks, so a dangling symlink at this path
        // would read as absent and a naive write would create the link's
        // target — potentially outside `out`. `symlink_metadata` sees the
        // link itself, so this must be treated as a foreign file: warned
        // about and left untouched, never written through.
        let tmp = tempfile::tempdir().unwrap();
        ...
        assert!(
            ...
            "the dangling symlink itself must survive untouched"
        );
        assert!(
            !target.exists(),
            "the render must never create the symlink's target"
        );
    }
